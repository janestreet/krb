...
module Cross_realm = struct
  ...
  let keep_valid
    ...
    | None ->
      ...
      if not !removed
      then
        Hashtbl.add_exn renewal_jobs ~key:renewal_key ~data:(`Wait ivar, extendable_abort);
      let%bind result =
...
let keep_valid_indefinitely ?refresh_every ?on_error ?keytab ?abort ~cred_cache principal =
  Principal.Name.with_default_realm principal
  >>= Cross_realm.keep_valid_indefinitely
        ?refresh_every
        ?on_error
        ?keytab
        ?abort
...
let num_active_renewal_jobs = Cross_realm.num_active_renewal_jobs
